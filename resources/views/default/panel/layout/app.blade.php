...
>
    ...
    @if (request()->is('login') && request()->has('confirmed'))
    {{-- 🔴 Register ke baad - RED --}}
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            var errorDiv = document.getElementById('errorsshow_login');
            if (errorDiv) {
                errorDiv.style.display = 'block';
                errorDiv.style.color = '#ef4444';
                errorDiv.style.textAlign = 'center';
                errorDiv.innerHTML = '<p>We have sent you an email for account confirmation. Please confirm your account to continue.</p>';
            }
        });
    </script>
@elseif (session()->has('message') && session('type') == 'success' && request()->is('login'))
    {{-- 🟢 Email confirm ke baad - GREEN --}}
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            var errorDiv = document.getElementById('errorsshow_login');
            if (errorDiv) {
                ...
                errorDiv.style.textAlign = 'center';
                errorDiv.innerHTML = '<p>{{ session("message") }}</p>';
            }
        });
    </script>
@elseif (session()->has('message'))
    {{-- Other toastr messages --}}
    <script>
