...
@section('settings')
    ...
    <input
        ...
        value="{{ $setting->get_started_text ?? '' }}"
    ...
    >
            ...
            <div class="col-md-12">
    <div class="mb-3">
        <label class="form-label">{{ __('Video URL (YouTube / Vimeo)') }}</label>
        <input
            class="form-control"
            id="video_url"
            type="text"
...
<div class="mb-3">
    <label class="form-label">{{ __('Intelligent AI tools ') }}</label>
    <input
        class="form-control"
        id="slider_heading"
        type="text"
        name="slider_heading"
        value="{{ $setting->slider_heading ?? '' }}"
        placeholder="Enter slider heading text"
    >
</div>

<div class="mb-3">
    <label class="form-label">{{ __('transform marketing') }}</label>
    <input
        class="form-control"
        id="transform_marketing"
        type="text"
        name="transform_marketing"
        value="{{ $setting->transform_marketing ?? '' }}"
        placeholder="Enter transform marketing text"
    >
</div>

<div class="mb-3">
    <label class="form-label">AI Powered Messaging Text</label>
    <input
        class="form-control"
        id="ai_powered_text"
        type="text"
        name="ai_powered_text"
        value="{{ $setting->ai_powered_text ?? '' }}"
        placeholder="Enter text (e.g. AI-powered messaging)"
    >
</div>
            ...
            <div class="col-md-12">
                <div class="mb-">
					...
					<div class="group relative w-full mt-4">
						<button
							class="mb-3 flex w-full items-center gap-5 text-sm text-heading-foreground"
							type="button"
						>
						...
						<x-forms.input
							...
							multiple
							type="file"
							placeholder="{{ __('Upload Slider Images') }}"
						/>
					</div>
...
<div class="my-4">
    <label class="form-label fw-bold">{{ __('Uploaded Slider Images') }}</label>
    <div class="d-flex flex-wrap gap-3 mt-2">
      @foreach($sliders as $slider)
<div style="position:relative; display:inline-block; margin:12px 8px 4px 4px;">
    <img 
    ...
    <button 
        type="button"
        onclick="openSliderDeleteModal({{ $slider->id }})"
        style="position:absolute;top:-8px;right:-8px;background:red;color:white;border:none;border-radius:50%;width:24px;height:24px;display:flex;align-items:center;justify-content:center;font-size:16px;line-height:1;cursor:pointer;padding:0;z-index:10;"
    >×</button>
</div>
@endforeach
    </div>
...
<div id="sliderDeleteModal" style="display:none; position:fixed; inset:0; background:rgba(0,0,0,0.5); z-index:9999; align-items:center; justify-content:center;">
    <div style="background:#fff; border-radius:12px; padding:2rem; max-width:320px; width:90%; text-align:center;">
        <!-- <p style="font-size:15px;font-weight:600;margin:0 0 0.5rem;color:#1a1a1a;">Image delete karein?</p>
        <p style="font-size:13px;color:#6b7280;margin:0 0 1.5rem;">Yeh action undo nahi ho sakta.</p> -->
        <div style="display:flex;gap:10px;">
            <button onclick="closeSliderDeleteModal()" style="flex:1;padding:10px;border-radius:8px;border:1px solid #dee2e6;background:#f8f9fa;color:#374151;cursor:pointer;">Cancel</button>
            <a id="sliderDeleteConfirmBtn" href="#" style="flex:1;padding:10px;border-radius:8px;background:red;color:#fff;text-decoration:none;display:flex;align-items:center;justify-content:center;font-weight:500;">Delete</a>
        </div>
    </div>
</div>

<script>
    function openSliderDeleteModal(id) {
        document.getElementById('sliderDeleteConfirmBtn').href = '/dashboard/admin/frontend/slider-delete/' + id;
        document.getElementById('sliderDeleteModal').style.display = 'flex';
    }
    function closeSliderDeleteModal() {
        document.getElementById('sliderDeleteModal').style.display = 'none';
    }
    document.addEventListener('keydown', function(e) {
        if (e.key === 'Escape') closeSliderDeleteModal();
    });
    document.getElementById('sliderDeleteModal').addEventListener('click', function(e) {
        if (e.target === this) closeSliderDeleteModal();
    });
</script>
@endif
                ...
                <div class="row mb-4">
                    ...
                    <div class="floating-button-input mt-2 flex items-center justify-center">
                        <a
                            ...
                            data-fslightbox="floating-video-gallery"
...
@push('script')
    ...
    <script>
		...
		if ($('#faster_content_creation').length) {
			var faster_content_creation = ace.edit("faster_content_creation");
			faster_content_creation.session.setMode("ace/mode/html");
		}

		if ($('#over_5000_businesses').length){
