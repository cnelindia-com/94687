...
@if ($item['show_condition'] && (!data_get($item, 'is_admin') || (auth()->check() && (auth()->user()->isAdmin() || auth()->user()->isSuperAdmin()))))

    @if ($item['show_condition'] && !(auth()->check() && !auth()->user()->isAdmin() && str_contains(strtolower($item['label']), 'fashion')))
        ...
        @php
            ...
            $is_active = $href === url()->current() || activeRoute(...(is_array($activeConditionArr) ? $activeConditionArr : []));
        @endphp
