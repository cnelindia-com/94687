...
class CheckTemplateTypeAndPlan
{
    ...
    {
        // if super_admin then allow everything
    ...
    private function performUserChecks(Request $request): bool
    {
        ...
        // =====================================================================
        // ✅ CREDITS-FIRST LOGIC:
        // Agar user ke paas credits hain (> 0), toh plan expire/null ho tab bhi
        // saare pages access milenge. Credits hi user ka "pass" hain.
        // =====================================================================
        $userCredits = (int) ($user?->total_credit ?? $user?->total_credits ?? 0);

        if ($userCredits <= 0) {
            // ── Credits ZERO hain ────────────────────────────────────────────
            // Sirf Support aur My Photoshoots allow karo, baaki sab block
            $allowedSlugs = [
                'support',
        ...
        // ── Credits > 0 hain ─────────────────────────────────────────────────
        // Plan expire/null ho toh bhi feature check karo aur allow karo
        ...
        // if openai record exist or slug is in the list of slugs not in openai generator
        if ($openAi || in_array($slug, $slugsNotInOpenAiGenerator, true)) {
            // Check if the feature is enabled in settings
            $setting = $this->settingSlug($slug);
            if ($setting['status']) {
                $settingValue = Helper::setting($setting['setting']);
                // abort if the feature is disabled globally
                if ($settingValue === 0) {
                    abort(404);
                }
            }

            // ✅ Credits hain → access allow karo (plan check bypass)
            return true;
        }
