  ...
  private function saveImageToLocal(array $imageDetails): string
{
    ...
    $content = $imageDetails['imageContent'];

    $watermark = app(\App\Services\Image\WatermarkService::class);
    if ($watermark->shouldApply()) {
        $content = $watermark->apply($content);
    }

    Storage::disk('public')->put($imageDetails['nameOfImage'], $content);
...
private function applyWatermark(string $imagePath): void
{
    ...
    if (! $image) {
        Log::warning('Watermark: image load failed', ['path' => $imagePath]);
        return;
    }
    ...
    // GD built-in font (no TTF needed)
    $font     = 5; // 1-5, 5 = sabse bada
    $text     = 'TRIAL IMAGE';
    ...
    // Background box banao taaki text visible ho
    ...
    // Save karo
    match ($mimeType) {
        'image/jpeg' => imagejpeg($image, $imagePath, 90),
        'image/webp' => imagewebp($image, $imagePath, 90),
        default      => imagepng($image, $imagePath),
    };

    imagedestroy($image);

    Log::info('Watermark applied', ['path' => $imagePath]);
}
