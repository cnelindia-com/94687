...
class AIImageController extends Controller
{
    ...
    public function generateImage(Request $request): JsonResponse
    {
        ...
        // ✅ Check if user is a team member with viewer role
        if ($user->team_id) {
            ...
            if ($teamMember && $teamMember->role === 'viewer') {
                return response()->json([
                    'message' => __('Viewers cannot generate images. Only creators can generate images.'),
                    'type'    => 'error',
                ], 403);
            }
        }
    ...
    public function imageOutput($param, $post, $user): JsonResponse
    {
        ...
        try {
            ...
            for ($i = 0; $i < $number_of_images; $i++) {
                ...
                $savePath = $this->saveImageOutputToStorage($imageDetails, $user);
    ...
    private function saveImageOutputToStorage(array $imageDetails, ?User $user = null): string
    {
        if (isset($imageDetails['engine']) && $imageDetails['engine'] === EngineEnum::FAL_AI) {
            return '/';
        }
        ...
        return match ($image_storage) {
            self::STORAGE_S3    => $this->saveImageToS3($imageDetails, $user),
            self::CLOUDFLARE_R2 => $this->saveImageToR2($imageDetails, $user),
            default             => $this->saveImageToLocal($imageDetails, $user),
        };
    }

    private function saveImageToS3(array $imageDetails, ?User $user = null): string
    {
        $localPath = $this->saveImageToLocal($imageDetails, $user);
    ...
    /**
     * Image local storage mein save karo.
     * Trial user hone par watermark apply hoga.
     */
    private function saveImageToLocal(array $imageDetails, ?User $user = null): string
    {
        
        $imageContent = $imageDetails['imageContent'] ?? '';

        $watermark = app(\App\Services\Image\WatermarkService::class);
        if ($watermark->shouldApply($user)) {
            $imageContent = $watermark->apply($imageContent);
        }

        Storage::disk('public')->put($imageDetails['nameOfImage'], $imageContent);

        return 'uploads/' . $imageDetails['nameOfImage'];
    }

    private function saveImageToR2(array $imageDetails, ?User $user = null): string
    {
        $imageContent = $imageDetails['imageContent'] ?? '';
        ...
        Storage::disk('r2')->put($imageDetails['nameOfImage'], $imageContent);
