...
class AdminController extends Controller
{
    ...
    public function usersEdit(User $user): View|RedirectResponse
    {
        ...
        $user->loadMissing('relationPlan');
        $currentPlanId = $user->relationPlan?->id;
        ...
        $plansSubscriptionMonthly = $plans
            ->where('frequency', FrequencyEnum::MONTHLY->value)
            ->sortBy('price');

        $plansSubscriptionAnnual = $plans
    ...
    public function usersSendPasswordReset(User $user): RedirectResponse
    {
        ...
        return $status === Password::RESET_LINK_SENT
            ? back()->with(['message' => __($status), 'type' => 'success'])
            : back()->with(['message' => __($status), 'type' => 'error']);
    }
...
public function usersSave(Request $request): \Illuminate\Http\JsonResponse
{
    if (Helper::appIsDemo()) {
        return response()->json(['message' => 'Demo mode'], 403);
    }
    ...
    if ($request->filled('plan_id')) {
        try {
            $this->adminAssignPlan($user, (int) $request->plan_id);
        } catch (\Throwable $e) {
            Log::error('adminAssignPlan failed: ' . $e->getMessage());
            return response()->json([
                'message' => 'User saved but plan update failed: ' . $e->getMessage(),
                'status'  => 'warning',
            ]);
        }
    }
...
private function adminAssignPlan(User $user, int $newPlanId): void
{
    // Step 1: Current active subscriptions dhundo
    $activeSub    = getCurrentActiveSubscription($user->id);
    $activeSubYok = getCurrentActiveSubscriptionYokkasa($user->id);

    // Step 2: Same plan hai to skip
    $currentPlanId = $activeSub?->plan_id ?? $activeSubYok?->plan_id ?? null;
    if ((int) $currentPlanId === $newPlanId) {
        Log::info("Same plan {$newPlanId}, skipping for user {$user->id}");
        return;
    }

    // Step 3: Old subscription cancel karo
    if ($activeSub) {
        $activeSub->stripe_status = 'bank_canceled';
        $activeSub->ends_at       = now();
        $activeSub->save();
        Log::info("Cancelled sub ID: {$activeSub->id} for user: {$user->id}");
    }

    if ($activeSubYok) {
        $activeSubYok->stripe_status = 'bank_canceled';
        $activeSubYok->ends_at       = now();
        $activeSubYok->save();
    }

    // Step 4: Naya plan fetch karo
    $newPlan = Plan::findOrFail($newPlanId);

    // Step 5: Unique order ID generate karo
    $orderId = 'TSO-' . strtoupper(Str::random(13));

    // Step 6: Naya subscription insert karo
    DB::table('subscriptions')->insert([
        ...
        'quantity'      => 1,
        'trial_ends_at' => null,
        'ends_at'       => $newPlan->frequency === 'monthly'
                            ? now()->addMonth()
                            : now()->addYear(),
        'paid_with'     => 'banktransfer',
        'tax_value'     => 0,
        'coupon'        => null,
        'total_amount'  => $newPlan->price ?? 0,
        'auto_renewal'  => 0,
        'created_at'    => now(),
        'updated_at'    => now(),
    ]);

    Log::info("Updating credits inside the form  Plan: ");

    // ✅ Step 7: User credits update karo
    $this->updateUserCreditsFromPlan($user, $newPlan);


    Log::info("Admin assigned plan {$newPlanId} to user {$user->id} - SUCCESS");
}

/**
 * ✅ Plan ke credits user ko assign karo
 */
private function updateUserCreditsFromPlan(User $user, Plan $newPlan): void
{
    try {
        $totalCredits = (int) ($newPlan->total_credits ?? 0);

        Log::info('🟡 [Credits Update Started]', [
            'user_id'   => $user->id,
            'plan_id'   => $newPlan->id,
            'plan_name' => $newPlan->name ?? null,
            'credits_from_plan' => $totalCredits,
        ]);
        ...
        $userEntityCredits = [];
        if ($freshUser && $freshUser->entity_credits) {
            $userEntityCredits = json_decode($freshUser->entity_credits, true) ?? [];
        }

        Log::info('📦 Existing Credits Loaded', [
            'user_id' => $user->id,
            'existing_total_credit' => $freshUser->total_credit ?? 0,
        ]);

        // Default model
        $defaultModel = $newPlan->default_ai_model ?? 'gpt-4o';
        ...
        foreach ($modelsToCredit as $provider => $model) {

            if (isset($userEntityCredits[$provider][$model])) {

                $oldCredit = $userEntityCredits[$provider][$model]['credit'] ?? 0;

                $newCredit = $oldCredit + $totalCredits;

                $userEntityCredits[$provider][$model]['credit'] = $totalCredits;

                Log::info('🔁 Model Credit Updated', [
                    'provider'    => $provider,
                    'model'       => $model,
                    'old_credit'  => $oldCredit,
                    'added_credit'=> $newCredit,
                    'new_credit'  =>  $newCredit,
                ]);

            } else {

                Log::warning('⚠️ Model Not Found in User Credits', [
                    'provider' => $provider,
                    'model'    => $model,
                ]);
            }
        ...
        DB::table('users')
            ->where('id', $user->id)
            ->update([
                'entity_credits' => json_encode($userEntityCredits),
        ...
        Log::info('✅ Credits Successfully Updated', [
            'user_id' => $user->id,
            'final_total_credit' => $totalCredits,
        ]);

    } catch (\Throwable $e) {

        Log::error('❌ Credits Update FAILED', [
...
//Sudheer End
    ...
    {
        $sliders = \Illuminate\Support\Facades\DB::table('banner_sliders')->orderBy('id', 'desc')->get();
    return view('panel.admin.frontend.settings', compact('sliders'));
    }

    public function sliderDelete($id)
{
    $slider = \Illuminate\Support\Facades\DB::table('banner_sliders')->where('id', $id)->first();
    if ($slider) {
        \Illuminate\Support\Facades\Storage::disk('public')->delete($slider->image);
        \Illuminate\Support\Facades\DB::table('banner_sliders')->where('id', $id)->delete();
    }
    return back()->with(['message' => 'Image deleted successfully', 'type' => 'success']);
}
    ...
    public function frontendSettingsSave(Request $request)
    {
        if (Helper::appIsNotDemo()) {
            ...
            $settings->slider_heading = $request->slider_heading;
            $settings->transform_marketing = $request->transform_marketing;
            $settings->ai_powered_text = $request->ai_powered_text;
            $settings->get_started_text = $request->get_started_text;
            $settings->save();
            ...
            if ($request->hasFile('hero_image')) {
                ...
                //     $data = [
                //         'errors' => ['The maximum allowed image size is 2MB.'],
                //     ];
...
if ($request->hasFile('slider_images')) {
    ...
    Log::info('Slider images upload started');

    $files = $request->file('slider_images');
    Log::info('Total files received: ' . count($files));
    ...
    foreach ($files as $index => $file) {
        ...
        try {
            $imageTypes = ['jpg', 'jpeg', 'png', 'svg', 'webp', "avif"];
            ...
            Log::info('Inserted into DB successfully', ['path' => $path]);
    ...
    Log::info('Slider images upload finished');

} else {
    Log::warning('No slider images found in request');
}
