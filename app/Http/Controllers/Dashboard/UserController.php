...
class UserController extends Controller
{
    ...
    {
        try {
            // Extract the file path from the output URL
            ...
            // If output contains a URL (http:// or https://), extract the path
            if (str_contains($output, 'https://') || str_contains($output, 'http://')) {
            ...
            if ($file) {
                // Determine storage disk and delete main file
                if ($workbook->storage == UserOpenai::STORAGE_LOCAL || $workbook->storage == 'public' || $workbook->storage == 'uploads') {
                ...
                $baseName = $pathInfo['filename'] ?? basename($file);
                $extension = $pathInfo['extension'] ?? '';
                
                // Construct thumbnail path based on URL structure
                // Original: /uploads/media/images/u-200/filename.png
                // Thumbnail: /uploads/thumbnail/default/filename_hash.webp
                $thumbnailFile = 'uploads/thumbnail/default/' . $baseName . '_' . md5($baseName) . '.webp';
                Storage::disk('public')->delete($thumbnailFile);
            }
        } catch (Throwable $th) {
            Log::error('Error deleting file from storage: ' . $th->getMessage());
        }
    }
    ...
    public function documentsImageDelete($slug)
    {
        ...
        try {
            ...
            Log::error('Image delete error: ' . $exception->getMessage());
        }
