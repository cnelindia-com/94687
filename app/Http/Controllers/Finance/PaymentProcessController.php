   ...
   public function startSubscriptionProcess($planId, $gatewayCode)
{
    ...
    if ($gatewayCode === 'walletmaxpay') {
        return back()->with([
            'message' => __('WalletMaxPay available only for Token Packs'),
            'type'    => 'error',
        ]);
    }

    $activeSub = getCurrentActiveSubscription() ?: getCurrentActiveSubscriptionYokkasa();

    if ($activeSub && (int) $activeSub->plan_id === (int) $planId) {
        return back()->with([
            'message' => __('You already have this subscription plan. Please choose a different plan to upgrade or downgrade.'),
            'type'    => 'error',
        ]);
    }
    ...
    $isTrialActive = false;
    if ($activeSub) {
        $currentPlan = Plan::find($activeSub->plan_id);
        $isTrialActive = $currentPlan &&
            $currentPlan->trial_days > 0 &&
            \Carbon\Carbon::now()->lessThanOrEqualTo(
                $activeSub->created_at->addDays($currentPlan->trial_days)
            );
    }

    // ✅ If any subscription is active, cancel it once before creating the new one
    if ($activeSub) {
        try {
            DB::beginTransaction();
            self::cancelActiveSubscription();
            DB::commit();
        } catch (Exception $e) {
            DB::rollBack();
            Log::error('Subscription Switch Cancellation Error: ' . $e->getMessage());
            return back()->with([
                'message' => __('Failed to cancel current subscription. Please try again.') . ' ' . $e->getMessage(),
                'type' => 'error'
            ]);
        }
    }

    // ✅ Normal flow - No trial active, proceed with payment
    try {
...
//                 $gatewayClass = GatewaySelector::selectGateway($gatewayCode);
// dump($gatewayCode, $gatewayClass, $plan);

// if (! $gatewayClass) {
//     throw new \Exception("Gateway class not resolved for code: {$gatewayCode}");
// }

// if (! method_exists($gatewayClass, 'prepaid')) {
//     throw new \Exception("Method 'prepaid' does not exist on: " . (is_object($gatewayClass) ? get_class($gatewayClass) : $gatewayClass));
// }
// die();
// return $gatewayClass::prepaid($plan);
   ...
   public static function assignPlanByAdmin(Request $request): null|RedirectResponse|View
{
    ...
    try {
        ...
        |--------------------------------------------------------------------------
        */
        $oldSubscriptions = Subscriptions::where('user_id', $user->id)
        ...
        foreach ($oldSubscriptions as $oldSub) {
            $oldSub->stripe_status = 'cancelled';
            $oldSub->ends_at = now();
            $oldSub->save();
        }
        ...
        */
        ...
        $order->affiliate_earnings =
            ($total * $settings->affiliate_commission_percentage) / 100;
        ...
        */
        ...
        return back()->with([
            'message' => __('The plan has been successfully assigned to the user.'),
            'type'    => 'success',
        ]);
        ...
        return back()->with([
            'message' => Str::before($ex->getMessage(), ':'),
            'type'    => 'error'
        ]);
    }
}
