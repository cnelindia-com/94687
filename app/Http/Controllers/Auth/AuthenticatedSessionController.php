...
class AuthenticatedSessionController extends Controller
{
    ...
    public function store(LoginRequest $request): \Illuminate\Http\JsonResponse
    {
        ...
        // Check if user is a suspended team member
        if ($user) {
            $suspendedMember = \App\Models\Team\TeamMember::where('user_id', $user->id)
                ->where('status', 'suspended')
                ->exists();
            if ($suspendedMember) {
                return response()->json([
                    'errors' => [__('Your account has been suspended. Please contact your team administrator.')],
                    'type'   => 'error',
                ], 401);
            }
        }
        ...
        if (Auth::check()) {
            ...
            // If a user was created outside the normal registration flow, ensure they can start trial on first login.
    ...
    private function getRedirectUrl(?string $redirect, ?string $plan): string
    {
        ...
        if (setting('hard_redirect_to_user_dashboard', '0') === '0' && Auth::user()?->isAdmin()) {
            return $plan ? '/dashboard/user/fashion-studio' : '/dashboard/admin';//Pritam
        }

        return $plan ? '/dashboard/user/fashion-studio' : '/dashboard/user';//Pritam
    }
