...
class AuthenticationController extends Controller
{
    ...
    public function registerStore(Request $request): JsonResponse
    {
        ...
        // ✅ For viewer role, keep credits as NULL; for creator role, use the allocated credits
        $allocatedCredits = null;
        if ($teamMember && $teamMember->role === 'creator') {
            $allocatedCredits = (int) ($teamMember?->remaining_words ?? 0);
        }
        ...
        DB::transaction(function () use (&$user, $teamMember, $allocatedCredits, $request, $name, $surname, $normalize, $affCode) {
            $user = User::create([
                ...
                'affiliate_code'          => Str::upper(Str::random(12)),
                'total_credit'           => $allocatedCredits,
            ]);

                $teammangerid=$teamMember?->team?->user_id;
            if (! $teamMember) {
                $user->updateCredits(setting('freeCreditsUponRegistration', User::getFreshCredits()));
                app(TrialPlanService::class)->assignTrialIfEligible($user);
            }
            ...
            // ✅ Record credit allocation in credits table for new user (only for creator role)
            if ($teamMember && $allocatedCredits > 0 && $teamMember->role === 'creator') {
                // $user->updateCredits(User::getFreshCredits($allocatedCredits));

                // DB::table('credits')->insert([
                //     'user_id'    => $user->id,
                //     'credits'    => $allocatedCredits,
                //     'types'      => 1,  // Addition type
                //     'action'     => 'team_invite_accept',
                //     'recordid'   => $teamMember->id,
                //     'created_at' => now(),
                // ]);

             // Add credits to new team member
             DB::table('credits')->insert([
                'user_id'    => $user->id,
                'credits'    => $allocatedCredits,
                'types'      => 1,
             ...
             // Deduct credits from team owner only once (at acceptance time)
             $ownerId = $teamMember?->team?->user_id;
             if ($ownerId) {
                 DB::table('credits')->insert([
                    'user_id'    => $ownerId,
                    'credits'    => $allocatedCredits,
                    'types'      => 2,
                    'action'     => "team_invite_accept_deduction",
                    'created_at' => now(),
                 ]);
                 DB::update("
                    UPDATE users u
    ...
    session()->flash('message', 'We have sent you an email for account confirmation. Please confirm your account to continue.');

    return response()->json([
        'errors' => ['We have sent you an email for account confirmation. Please confirm your account to continue.'],
        'type'   => 'confirmation',
        'redirect' => route('login'), // ✅ Login page ka URL
    ], 401);
}
