    ...
    private function saveImageToLocal(array $imageDetails): string
{
    ...
    $debugUser = Auth::user();
    \Log::info('WATERMARK_DEBUG', [
        'user_id'       => $debugUser?->id,
        'trial_ends_at' => $debugUser?->trial_ends_at,
        'now'           => now()->toDateTimeString(),
        'is_future'     => $debugUser?->trial_ends_at ? now()->lt($debugUser->trial_ends_at) : false,
        'content_size'  => strlen($content ?? ''),
    ]);
    
    $watermark = app(\App\Services\Image\WatermarkService::class);
    if ($watermark->shouldApply(Auth::user())) {
