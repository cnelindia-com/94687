...
@endsection
@section('titlebar_subtitle', __('Swap out the current model for the desired image to set up a fresh photoshoot.'))

@section('content')
    <div
        class="py-10"
        x-data="changeModelApp()"
    >
        <div class="flex flex-col gap-4 lg:flex-row lg:gap-9">
            ...
            <div class="w-full lg:basis-1/3">
                ...
                <form
                    ...
                    @submit.prevent="generateChangeModel"
                >
                    @csrf

                    {{-- Image of the Model to be Replaced --}}
                    <div>
                        <label class="mb-4 flex items-center gap-1 text-2xs font-medium">
                            {{ __('Image of the Model to be Replaced') }}
                            <x-info-tooltip
                                class:content="-start-4 translate-x-0 text-2xs"
                                text="{{ __('Upload an image featuring the model you wish to replace.') }}"
                            />
                        </label>
                        ...
                        <input
                            ...
                            @change="handleFileSelect($event, 'original')"
                        >
                        ...
                        <div
                            ...
                            @click="$refs.originalInput.click()"
                            @dragover.prevent="originalDragOver = true"
                            @dragleave.prevent="originalDragOver = false"
                            @drop.prevent="handleFileDrop($event, 'original')"
                            :class="{ 'drag-over': originalDragOver }"
                        >

                            <div x-show="!originalPreview">
                                <svg
                            ...
                            <div
                                class="relative mx-auto mb-2 inline-block w-32"
                                x-show="originalPreview"
                                x-cloak
                            >
                                <img
                    ...
                    <div>
                        <label class="mb-4 flex items-center gap-1 text-2xs font-medium">
                            ...
                            <x-info-tooltip
                                class:content="-start-4 translate-x-0 text-2xs"
                                text="{{ __('Upload an image of the new model you want to feature.') }}"
                            />
                        </label>
                        ...
                        <input
                            ...
                            x-ref="newModelInput"
                            accept="image/*,.heic,.heif"
                            @change="handleFileSelect($event, 'newModel')"
                        >

                        <div
                            class="cursor-pointer rounded-[10px] border border-dashed border-foreground/10 p-6 text-center transition hover:border-primary hover:bg-primary/5 sm:p-8"
                            @click="$refs.newModelInput.click()"
                            @dragover.prevent="newModelDragOver = true"
                            @dragleave.prevent="newModelDragOver = false"
                            @drop.prevent="handleFileDrop($event, 'newModel')"
                            :class="{ 'drag-over': newModelDragOver }"
                        >
                            <div x-show="!newModelPreview">
                                <svg
                                    class="mx-auto mb-2.5 opacity-25"
                                    width="38"
                                    height="38"
                    ...
                    <x-button
                        ...
                        variant="primary"
                        disabled
                        ::disabled="generating || !originalFile || !newModelFile"
                    >
                        {{ __('Generate') }}
                    </x-button>
                </form>
            ...
            <div class="flex w-full flex-col lg:basis-2/3">
                <div class="mb-5 flex flex-wrap justify-between gap-3 border-b py-2.5 transition-border">
                    ...
                    >
                        {{ __('Download All') }}
                        <x-tabler-download class="ml-2 size-4" />
                    </x-button>
                </div>
...
@push('script')
    <script>
        const ROUTES = {
            generate: '{{ route('dashboard.user.fashion-studio.change_model.generate') }}',
            checkStatus: '{{ url('dashboard/user/fashion-studio/change_model/status') }}',
        };

        function changeModelApp() {
            return {
                // File state
                originalFile: null,
                originalPreview: null,
                originalFileName: null,
                originalDragOver: false,

                newModelFile: null,
                newModelPreview: null,
                newModelFileName: null,
                newModelDragOver: false,
                ...
                handleFileDrop(event, type) {
                    if (type === 'original') {
                        this.originalDragOver = false;
                    } else {
                        this.newModelDragOver = false;
                    }
                ...
                processFile(file, type) {
                    ...
                    reader.onload = (e) => {
                        if (type === 'original') {
                            ...
                            this.newModelPreview = e.target.result;
                            this.newModelFileName = file.name;
                        }
                    };
                ...
                resetFile(type) {
                    if (type === 'original') {
                        ...
                        this.newModelPreview = null;
                        this.newModelFileName = null;
                        if (this.$refs.newModelInput) {
                            this.$refs.newModelInput.value = '';
                        }
                    }
                },

                // Generate change model
                async generateChangeModel() {
                    if (!this.originalFile || !this.newModelFile) {
                        toastr.warning("{{ __('Please upload both original and new model images') }}");
                        return;
                    }
                    ...
                    formData.append('original_image', this.originalFile);
                    formData.append('new_model_image', this.newModelFile);
                    ...
                    try {
                        ...
                        toastr.error(error.message || "{{ __('Failed to change model') }}");
                    ...
                    const checkInterval = setInterval(async () => {
                        ...
                        try {
                            ...
                            if (data.status?.toLowerCase() === 'completed') {
                                ...
                                toastr.success("{{ __('Model changed successfully!') }}");

                                // Store results
                                this.results = data.results || [data.change_model];
                ...
                downloadAll() {
                    this.results.forEach((result, index) => {
                        ...
                        link.download = `change-model-${index + 1}.png`;
