...
@section('title', __('Default Models'))
@section('titlebar_subtitle', __('Manage your default AI models.'))

@section('settings')
    <div class="space-y-6">
        {{-- Header --}}
        <div>
            <h3 class="text-2xl font-bold text-gray-900 dark:text-white">{{ __('Default Models') }}</h3>
            <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">{{ __('Manage your default AI models') }}</p>
        </div>
        ...
        <div id="tab-content-default">
            <p class="text-xs text-gray-400 dark:text-gray-500 mb-4 text-center">
                💡 {{ __('Drag and drop models to reorder. Order saves automatically to database.') }}
            </p>
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6" id="models-grid">
                @forelse($results as $model)
<div class="model-card group bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 overflow-hidden shadow-sm cursor-grab select-none"                         data-id="{{ $model->id }}"
                         draggable="true">
                         <div class="relative">
    <button
        onclick="deleteModel({{ $model->id }}, this)"
    ...
    >
        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/>
                        ...
                        <div class="aspect-[3/4] overflow-hidden pointer-events-none">
                            <img src="{{ $model->thumbnail ?? $model->image_url }}"
                                 alt="{{ $model->name }}"
                                 class="w-full h-full object-cover">
                        </div>
                        <div class="p-4 pointer-events-none">
                            <h4 class="text-lg font-semibold text-gray-900 dark:text-white">{{ $model->name }}</h4>
                            <p class="text-sm text-gray-500">{{ $model->gender }}</p>
                        </div>
                    </div>
                @empty
                    <div class="col-span-full text-center py-10 text-gray-500">{{ __('No models found') }}</div>
                @endforelse
            </div>
        </div>
        ...
        <div id="tab-content-create" class="hidden">
            <form id="create-model-form" method="POST" action="{{ route('fashion-studio.save-custom-model') }}" enctype="multipart/form-data">
                @csrf
                    ...
                    <div class="p-6 space-y-6">
                        ...
                        <div>
                            <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                                {{ __('Model Name') }} <span class="text-red-500">*</span>
                            </label>
                        ...
                        <div>
                            ...
                            <div class="flex gap-6">
                                <label class="flex items-center gap-2 cursor-pointer">
                                    <input type="radio" name="gender" value="Male" required class="w-4 h-4 text-primary focus:ring-primary">
                                    <span class="text-sm text-gray-700 dark:text-gray-300">{{ __('Male') }}</span>
                                </label>
                                <label class="flex items-center gap-2 cursor-pointer">
                                    <input type="radio" name="gender" value="Female" required class="w-4 h-4 text-primary focus:ring-primary">
                                    <span class="text-sm text-gray-700 dark:text-gray-300">{{ __('Female') }}</span>
                                </label>
                            </div>
                        </div>
                        ...
                        <button type="submit" id="submit-btn"
                            ...
                            <span id="submit-btn-text">{{ __('Create New Model') }}</span>
                        </button>
                    </div>
                </div>
            </form>
    ...
    <style>
        ...
        .preview-item { transition: all 0.2s ease; cursor: grab; }
        .preview-item:active { cursor: grabbing; }

        .model-card { transition: opacity 0.2s ease, transform 0.2s ease, box-shadow 0.2s ease; }
        .model-card:active { cursor: grabbing; }
        .model-card.dragging  { opacity: 0.35; transform: scale(0.96); }
        .model-card.drag-over { box-shadow: 0 0 0 3px #10b981; transform: scale(1.02); }
        ...
        #toast.show { display: flex; animation: slideIn 0.35s ease forwards; }
    ...
    <script>
        ...
        // ===== DRAG & DROP — DATABASE RANK SAVE =====
        const grid = document.getElementById('models-grid');
        let dragSrc = null;

        function deleteModel(id, btn) {
    if (!confirm('Are you sure you want to delete this model?')) return;
    const card = btn.closest('.model-card');
    fetch(`/fashion-studio/delete-model/${id}`, {
        method: 'DELETE',
        headers: {
    ...
    .catch(() => showToast('Network error.', 'error'));
        ...
        function onDragEnd() {
            ...
            grid.querySelectorAll('.model-card').forEach(c => c.classList.remove('drag-over'));
            dragSrc = null;
        }
        ...
        function onDrop(e) {
            ...
            // Reorder DOM
            const allCards = [...grid.querySelectorAll('.model-card')];
        ...
        function saveRankToDatabase() {
            const ranks = [...grid.querySelectorAll('.model-card')].map((card, index) => ({
                id:   parseInt(card.getAttribute('data-id')),
                rank: index + 1
            }));

            fetch('{{ route("fashion-studio.update-rank") }}', {
                method:  'POST',
                headers: {
            ...
            .then(data => {
                showToast(
                    data.success
                        ? '{{ __("Order saved!") }}'
                        : (data.message || '{{ __("Failed to save order.") }}'),
                    data.success ? 'success' : 'error'
                );
            })
            .catch(() => showToast('{{ __("Network error. Order not saved.") }}', 'error'));
        }

        initDragDrop();

        // ===== CREATE TAB DROP ZONE =====
        ...
        dropZone.addEventListener('drop', e => {
            ...
            if (e.dataTransfer.files.length > 0) { fileInput.files = e.dataTransfer.files; showPreview(e.dataTransfer.files); }
        });
        ...
        // ===== FORM SUBMIT =====
        document.getElementById('create-model-form').addEventListener('submit', async function (e) {
            e.preventDefault();
            const submitBtn = document.getElementById('submit-btn');
            const btnText   = document.getElementById('submit-btn-text');

            if (!fileInput.files || fileInput.files.length === 0) {
                showToast('{{ __("Please upload at least one photo.") }}', 'error');
                return;
            }

            submitBtn.disabled = true;
            btnText.textContent = '{{ __("Uploading...") }}';
            ...
            try {
                ...
                if (response.ok) {
                    showToast(data?.message || '{{ __("Model created successfully!") }}', 'success');
                    setTimeout(() => location.reload(), 2000);
                } else {
                    let errMsg = '{{ __("Something went wrong. Please try again.") }}';
                    if (data?.message) errMsg = data.message;
                    else if (data?.errors) errMsg = Object.values(data.errors).flat().join(' ');
                    showToast(errMsg, 'error');
                    submitBtn.disabled = false;
                    btnText.textContent = '{{ __("Create New Model") }}';
                }
            } catch (err) {
