...
@section('title', __('My Photoshoots'))
@section('titlebar_pretitle', '')
@section('titlebar_actions')
    <x-button
        href="{{ route('dashboard.user.fashion-studio.wardrobe.index') }}"
...
@section('content')
    ...
    >
        ...
        <div class="mb-8 flex flex-wrap gap-4">
            <button
                class="active rounded-full px-3 py-1.5 text-2xs font-medium leading-none transition [&.active]:bg-foreground/10"
                type="button"
            ...
            <button
                class="rounded-full px-3 py-1.5 text-2xs font-medium leading-none transition [&.active]:bg-foreground/10"
                type="button"
        ...
        <p
            x-show="!loading && todayImages.length === 0 && yesterdayImages.length === 0 && olderImages.length === 0"
            x-cloak
        >
            <span x-text="currentFilter === 'videos' ? '{{ __('No videos found') }}' : '{{ __('No images found') }}'"></span>
        </p>

        <template x-if="!loading && (todayImages.length > 0 || yesterdayImages.length > 0 || olderImages.length > 0)">
            {{-- Image Sections --}}
            <template
                x-for="section in [{images: todayImages, title: '{{ __('Today') }}'}, {images: yesterdayImages, title: '{{ __('Yesterday') }}'}, {images: olderImages, title: '{{ __('Older') }}'}]"
                :key="section.title"
            >
                <div
                    class="mb-10"
                    x-show="section.images.length > 0"
                >
                    <h3
                                ...
                                <div class="relative size-full overflow-hidden">
                                    ...
                                    <template x-if="!image.is_video && !image.is_processing">
                                        <img
                                            ...
                                            :alt="image.title || image.input"
                                            loading="lazy"
                                            @click="!image.is_processing && (setActiveItem({...image, output: image.url}), modalShow = true)"
                                        >
                                    </template>
                                    ...
                                    <template x-if="image.parsedPayload?.product_urls?.length > 0 && !image.is_processing">
                                        <div
                                            class="absolute bottom-3.5 start-3.5 flex gap-1 rounded-lg bg-background p-1 opacity-0 transition-opacity duration-300 group-hover:opacity-100">
                                            <template
                                     ...
                                     <!-- ⭐ Creator Name Badge -->
<!-- <div
    class="absolute bottom-2 left-2 z-20 flex items-center gap-1.5 rounded-full bg-black/75 px-3 py-1.5 text-xs text-white backdrop-blur-sm"
    x-show="image.created_by_name && image.created_by_name !== 'Unknown' && image.created_by_name !== ''"
    x-cloak
>
    
    <template x-if="image.created_by_avatar">
        <img
            class="size-5 rounded-full object-cover"
            :src="image.created_by_avatar"
            :alt="image.created_by_name"
            loading="lazy"
        />
    </template>
    
    
    <span class="font-medium" x-text="image.created_by_name"></span>
</div> -->

                                     {{-- Hover overlay (hide for processing) --}}
                                     <div
                                        ...
                                        >
                                            ...
                                            >
                                                <ul class="py-1 text-xs font-medium">
                                                    ...
                                                    @endif
                                                    @if(!$isViewer)
                                                    <template x-if="image.can_delete">
                                                        <li>
                                                            <a
                                                                class="text-heading-foreground/2 flex px-5 py-2 transition-colors hover:bg-heading-foreground/[3%]"
                                                                href="javascript:void(0);"
    ...
    x-cloak
>
    <x-tabler-loader-2 class="size-5 animate-spin mr-2" />
    <span class="text-sm">{{ __('Loading more images...') }}</span>
</div>
...
@push('script')
    ...
    <script>
        const ROUTES = {
            ...
            downloadImage: '{{ route('dashboard.user.fashion-studio.photo_shoots.images.download', ['id' => 0]) }}',
...
handleScroll() {
    ...
    if (this.activeItem.created_by_name && this.activeItem.created_by_name !== 'Unknown' && this.activeItem.created_by_name !== '') {
        details.push({
            label: '{{ __('Created By') }}',
            value: this.activeItem.created_by_name
        });
    }
    ...
    return details;
                ...
                async loadImages(silent = false) {
                    ...
                    try {
                        const url = new URL(ROUTES.loadImages, window.location.origin);
                        ...
                        const today = images.filter(img => img.is_today);
const yesterday = images.filter(img => img.is_yesterday);
const older = images.filter(img => img.is_older);

if (this.page === 1) {
    this.todayImages = today;
    this.yesterdayImages = yesterday;
    this.olderImages = older;
} else {
    this.todayImages.push(...today);
    this.yesterdayImages.push(...yesterday);
    this.olderImages.push(...older);
}

this.hasMore = images.length >= this.perPage;
this.loadingMore = false;

                        const hasProcessing = images.some(img => img.is_processing);
                        if (hasProcessing && !this.refreshInterval) {
                ...
                async saveCroppedImage() {
                    ...
                    try {
                        ...
                        // Add new cropped image to today's images with updated payload
                        const newPayload = {
                ...
                downloadImage(image) {
                    // Use server-side download endpoint for proper Content-Disposition headers
                    const downloadUrl = ROUTES.downloadImage.replace('/0', '/' + image.id);
