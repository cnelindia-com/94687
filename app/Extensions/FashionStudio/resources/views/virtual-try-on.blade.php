            ...
            <div class="w-full lg:basis-1/3">
                ...
                >
                    ...
                    <div>
                        ...
                        >
                            <div x-show="!modelPreview && !modelConvertingHeic">
                                <svg
                                ...
                                <p class="mt-2 text-4xs font-medium">
                                    {{ __('PNG, JPG or HEIC') }}
                                </p>
                            </div>

                            <div x-show="modelConvertingHeic" x-cloak>
                                <div class="text-center py-4">
                                    <div class="mx-auto mb-3 w-12 h-12 rounded-full bg-blue-50 flex items-center justify-center">
                                        <svg class="animate-spin text-blue-500" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                                            <circle cx="12" cy="12" r="10"></circle>
                                            <path d="M12 6v6l4 2"></path>
                                        </svg>
                                    </div>
                                    
                                    <p class="text-4xs opacity-60">Please wait</p>
                                </div>
                            </div>

                            <div
                                x-show="modelPreview && !modelConvertingHeic"
                                x-cloak
                            >
                                <div class="relative mx-auto mb-2 inline-block w-32">
                                    <img
                                        class="max-h-40 w-full rounded-lg border object-contain bg-gray-50"
                                        :src="modelPreview"
                                    >
                                    <button
                                ...
                                ></p>
                                <p
                                    x-show="modelHeicConverted"
                                    class="mt-1 text-4xs font-medium "
                    ...
                    <div>
                        ...
                        >
                            <div x-show="!clothesPreview && !clothesConvertingHeic">
                                <svg
                                    class="mx-auto mb-2.5 opacity-25"
                                    width="38"
                                    height="38"
                            ...
                            <div x-show="clothesConvertingHeic" x-cloak>
                                <div class="text-center py-4">
                                    <div class="mx-auto mb-3 w-12 h-12 rounded-full bg-blue-50 flex items-center justify-center">
                                        <svg class="animate-spin text-blue-500" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                                            <circle cx="12" cy="12" r="10"></circle>
                            ...
                            <div
                                x-show="clothesPreview && !clothesConvertingHeic"
                                x-cloak
                            >
                                <div class="relative mx-auto mb-2 inline-block w-32">
                                    <img
                                ...
                                ></p>
                                <p
                                    x-show="clothesHeicConverted"
                                    class="mt-1 text-4xs font-medium text-green-600"
                    ...
                    >
                        <div class="group relative">
                            ...
                            <div class="absolute inset-0 flex items-center justify-center rounded-lg bg-black/10 opacity-0 transition-opacity group-hover:opacity-100">
                                <x-button
                                    @click.prevent="downloadImage(result.image_url, index)"
                                >
                                    {{ __('Download') }}
                                </x-button>
                            </div>
...
<script>
    const ROUTES = {
        ...
        preview: '{{ route('dashboard.user.fashion-studio.virtual_try_on.preview') }}',
        proxyImage: '{{ route('dashboard.user.fashion-studio.proxy.image') }}',
    };
    ...
    function virtualTryOnApp() {
        return {
            ...
            modelFileName: null,
            modelDragOver: false,
            modelConvertingHeic: false,
            modelHeicConverted: false,
            ...
            clothesConvertingHeic: false,
            clothesHeicConverted: false,
            ...
            // HEIC to JPG conversion function
            // HEIC to JPG conversion function
 async convertHeicToJpg(file) {
    const fileExt = file.name.split('.').pop().toLowerCase();

    if (fileExt !== 'heic' && fileExt !== 'heif') {
        return file; // seedha File object
    }
    ...
    try {
        ...
        return jpegFile; // seedha File object

    } catch (error) {
        console.warn('HEIC conversion failed:', error?.message);
        // Yahan tag laga do taaki caller ko pata chale server-fallback chahiye
        file._needsServerConversion = true;
        return file; // original file, marked
    }
},

async processFile(file, type) {
    const ext = (file.name || '').split('.').pop().toLowerCase();
    const isHeic = ext === 'heif' || ext === 'heic';
    ...
    if (file.size > 25 * 1024 * 1024) {
        toastr.error("{{ __('File size must be less than 25MB') }}");
        return;
    }

    // Always keep the ORIGINAL file for actual form submission —
    // backend already accepts heic/heif directly, no need to depend on preview conversion for upload
    if (type === 'model') {
        this.modelFile = file;
    ...
    if (!isHeic) {
        // Normal image — simple preview, no conversion needed
        const reader = new FileReader();
        reader.onload = (e) => {
            if (type === 'model') {
                this.modelPreview = e.target.result;
                this.modelHeicConverted = false;
            } else {
                this.clothesPreview = e.target.result;
                this.clothesHeicConverted = false;
            }
        };
    ...
    // ---- HEIC handling ----
    if (type === 'model') {
        this.modelConvertingHeic = true;
        this.modelPreview = null;
    } else {
        this.clothesConvertingHeic = true;
        this.clothesPreview = null;
    }

    let previewSet = false;

    // Attempt 1: client-side conversion
    try {
        const jpegFile = await this.convertHeicToJpg(file);
        const reader = new FileReader();
        await new Promise((resolve, reject) => {
            reader.onload = (e) => {
                if (type === 'model') {
                    this.modelPreview = e.target.result;
                    this.modelHeicConverted = true;
                } else {
                    this.clothesPreview = e.target.result;
                    this.clothesHeicConverted = true;
                }
                previewSet = true;
                resolve();
            };
        ...
        console.warn('[HEIC DEBUG] Client-side conversion failed, trying server preview endpoint...', clientError);

        // Attempt 2: server-side preview conversion
        try {
            const fd = new FormData();
            fd.append('file', file);
            fd.append('_token', document.querySelector('input[name="_token"]').value);

            const resp = await fetch(ROUTES.preview, {
                method: 'POST',
                body: fd,
                headers: {
            ...
            console.log('[HEIC DEBUG] Server preview response:', pd);

            if (pd?.url && !pd.url.match(/\.(heic|heif)($|\?)/i)) {
                // Only trust it if the returned URL is NOT still a heic/heif file
                if (type === 'model') {
                    this.modelPreview = pd.url;
                    this.modelHeicConverted = true;
                } else {
                    this.clothesPreview = pd.url;
                    this.clothesHeicConverted = true;
                }
                previewSet = true;
            } else {
                console.warn('[HEIC DEBUG] Server also returned a heic/heif URL — server-side conversion not actually happening.');
            }
        } catch (serverError) {
            console.error('[HEIC DEBUG] Server preview endpoint failed:', serverError);
        }
    }

    if (type === 'model') {
        this.modelConvertingHeic = false;
    } else {
        this.clothesConvertingHeic = false;
    }

    // Attempt 3: guaranteed-safe fallback — never show a broken image icon
    if (!previewSet) {
        console.warn('[HEIC DEBUG] All conversion attempts failed — showing placeholder instead of broken image.');
        const placeholder = this.buildHeicPlaceholderSvg(file.name);
        if (type === 'model') {
            this.modelPreview = placeholder;
            this.modelHeicConverted = false;
        } else {
            this.clothesPreview = placeholder;
            this.clothesHeicConverted = false;
        }
        toastr.warning("{{ __('Preview not available for this HEIC file, but it will still upload correctly.') }}");
    }
},

// Generates a simple inline SVG placeholder so the user at least sees a clean icon instead of a broken image
buildHeicPlaceholderSvg(fileName) {
    const svg = `
        <svg xmlns="http://www.w3.org/2000/svg" width="160" height="160" viewBox="0 0 160 160">
            <rect width="160" height="160" fill="#f3f4f6"/>
            <path d="M50 50h60v60H50z" fill="none" stroke="#9ca3af" stroke-width="3"/>
            <circle cx="65" cy="65" r="6" fill="#9ca3af"/>
            <path d="M50 100l20-20 15 15 15-20 10 25" fill="none" stroke="#9ca3af" stroke-width="3"/>
            <text x="80" y="135" font-size="11" text-anchor="middle" fill="#6b7280" font-family="sans-serif">HEIC</text>
        </svg>`;
    return 'data:image/svg+xml;base64,' + btoa(svg);
            ...
            handleFileSelect(event, type) {
                ...
                if (file) {
                    ...
                    if (file.size > maxSize) {
                        toastr.error("{{ __('File size should not exceed 25 MB') }}");
                        event.target.value = "";
                        return;
                    }
            ...
            async processFile(file, type) {
                ...
                if (isHeic) {
                    ...
                    try {
                        finalFile = await this.convertHeicToJpg(file);
                        converted = true;
                    } catch (error) {
                        console.warn('Client-side HEIC conversion failed, attempting server preview upload', error);

                        // Try server-side conversion for preview (falls back to server conversion if Imagick available)
                        try {
                            const fd = new FormData();
                            fd.append('file', file);
                            fd.append('_token', document.querySelector('input[name="_token"]').value);
                            ...
                            if (pd?.url) {
                                if (type === 'model') {
                                    this.modelPreview = pd.url;
                                    this.modelFileName = file.name.replace(/\.(heic|heif)$/i, '.jpg');
                                    this.modelFile = file; // keep original so server re-converts on final submit
                                    this.modelHeicConverted = true;
                                } else {
                                    this.clothesPreview = pd.url;
                                    this.clothesFileName = file.name.replace(/\.(heic|heif)$/i, '.jpg');
                                    this.clothesFile = file;
                                    this.clothesHeicConverted = true;
                                }
                                // Skip client-side preview since we already set preview URL
                                if (type === 'model') {
                                    this.modelConvertingHeic = false;
                                } else {
                                    this.clothesConvertingHeic = false;
                                }
                                // Do not run reader for server-preview case
                                return;
                            }
                        } catch (e) {
                            console.warn('Server preview upload failed', e);
                        }

                        toastr.error(error.message || 'HEIC conversion failed');
                        if (type === 'model') {
                            this.modelConvertingHeic = false;
                        } else {
                            this.clothesConvertingHeic = false;
                ...
                reader.onload = (e) => {
                    if (type === 'model') {
                        this.modelFile = finalFile;
                        this.modelPreview = e.target.result;
                        this.modelFileName = finalFile.name;
                    } else {
                        this.clothesFile = finalFile;
                        this.clothesPreview = e.target.result;
                        this.clothesFileName = finalFile.name;
                    }
                };
            ...
            // Download single image using proxy route (handles S3/Spaces same-origin requirement)
            async downloadImage(url, index) {
                const proxyUrl = ROUTES.proxyImage + '?url=' + encodeURIComponent(url);
            ...
            async downloadAll() {
                for (let index = 0; index < this.results.length; index++) {
                    await this.downloadImage(this.results[index].image_url, index);
                }
            }
        };
    }
