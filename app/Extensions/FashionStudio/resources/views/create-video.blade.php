                    ...
                    >
                        <div class="group relative">
                            ...
                            <div class="absolute inset-0 flex items-center justify-center rounded-lg bg-black/10 opacity-0 transition-opacity group-hover:opacity-100">
                                <x-button
                                    @click.prevent="downloadImage(result.video_url || result.image_url, index, result.video_url ? 'mp4' : 'png')"
                                >
                                    {{ __('Download') }}
                                </x-button>
                            </div>
...
@push('script')
    ...
    <script>
        const ROUTES = {
            generate: '{{ route('dashboard.user.fashion-studio.create_video.generate') }}',
            checkStatus: '{{ url('dashboard/user/fashion-studio/create_video/status') }}',
        ...
        function createVideoApp() {
            ...
            return {
                ...
                init() {
                    const createVideoData = sessionStorage.getItem('createVideoData');
                    if (createVideoData) {
                        try {
                            const data = JSON.parse(createVideoData);
                            this.loadImageFromUrl(data.url, data.fileName);
                            sessionStorage.removeItem('createVideoData');
                        } catch (error) {
    ...
    try {
        ...
        return file; // original file, marked
                ...
                async processFile(file) {
                    ...
                    // If it's a HEIC file, convert it to JPG in browser
                    if (isHeif) {
                        try {
                            const convertedFile = await this.convertHeicToJpg(file);
                            this.imageFile = convertedFile;
                            this.imageFileName = convertedFile.name;
                ...
                async generateVideo() {
                    if (!this.imageFile) {
                        toastr.warning("{{ __('Please upload a start image') }}");
                        return;
                    }
                    ...
                    try {
                        ...
                        toastr.error(error.message || "{{ __('Failed to create video') }}");
                ...
                checkGenerationStatus(id) {
                    ...
                    const checkInterval = setInterval(async () => {
                        ...
                        try {
                            ...
                            if (status === 'completed') {
                                ...
                                toastr.success("{{ __('Video created successfully!') }}");
                    ...
                    // Timeout after 6 minutes (video generation takes longer)
                    setTimeout(() => {
                        if (isCompleted) return;
                        isCompleted = true;
                        clearInterval(checkInterval);
                ...
                // Download single video/image using proxy route (handles S3/Spaces same-origin requirement)
                async downloadImage(url, index, extension) {
                    const proxyUrl = ROUTES.proxyImage + '?url=' + encodeURIComponent(url);
                    const link = document.createElement('a');
                    link.href = proxyUrl;
                    link.download = `generated-video-${index + 1}.${extension || 'mp4'}`;
                ...
                async downloadAll() {
                    for (let index = 0; index < this.results.length; index++) {
                        ...
                        const url = result.video_url || result.image_url;
                        const extension = result.video_url ? 'mp4' : 'png';
                        await this.downloadImage(url, index, extension);
                    }
                }
            };
        }
