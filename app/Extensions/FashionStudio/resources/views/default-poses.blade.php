...
@section('title', __('Default Poses'))

@section('titlebar_subtitle', __('Manage your default AI poses.'))

@section('settings')
    <div class="space-y-6">
        {{-- Header --}}
        <div>
            <h3 class="text-2xl font-bold text-gray-900 dark:text-white">{{ __('Default Poses') }}</h3>
            <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">{{ __('Manage your default AI poses') }}</p>
        </div>
        ...
        <div id="tab-content-default">
            <p class="text-xs text-gray-400 dark:text-gray-500 mb-4 text-center">
                💡 {{ __('Drag and drop poses to reorder. Order saves automatically to database.') }}
            </p>
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6" id="poses-grid">
                @forelse($poses as $pose)
                    <div class="pose-card group bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 overflow-hidden shadow-sm cursor-grab select-none"
                        data-id="{{ $pose->id }}" draggable="true">
                        <div class="relative">
                            <button onclick="deletePose({{ $pose->id }}, this)"
                        ...
                        <div class="aspect-[3/4] overflow-hidden pointer-events-none">
                            <img src="{{ $pose->thumbnail ?? $pose->image_url }}" alt="{{ $pose->name }}"
                                class="w-full h-full object-cover">
                        </div>
                        <div class="p-4 pointer-events-none">
                            <h4 class="text-lg font-semibold text-gray-900 dark:text-white">{{ $pose->name }}</h4>
                           
                        </div>
                    </div>
                @empty
                    <div class="col-span-full text-center py-10 text-gray-500">{{ __('No poses found') }}</div>
                @endforelse
            </div>
        </div>
        ...
        <div id="tab-content-create" class="hidden">
            <form id="create-pose-form" method="POST" action="{{ route('fashion-studio.save-custom-pose') }}"
                    ...
                    <div class="p-6 space-y-6">
                        ...
                        <div>
                            <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                                {{ __('Pose Name') }} <span class="text-red-500">*</span>
                            </label>
                            <input type="text" name="name" id="pose-name" placeholder="{{ __('e.g., Standing Pose') }}"
                        ...
                        <button type="submit" id="submit-btn"
                            ...
                            <span id="submit-btn-text">{{ __('Create New Pose') }}</span>
                        </button>
                    </div>
                </div>
            </form>
    ...
    <script>
        ...
        function onDragEnd() {
            ...
            grid.querySelectorAll('.pose-card').forEach(c => c.classList.remove('drag-over'));
            dragSrc = null;
        }
        ...
        function onDrop(e) {
            ...
            // Reorder DOM
            const allCards = [...grid.querySelectorAll('.pose-card')];
        ...
        function saveRankToDatabase() {
            const ranks = [...grid.querySelectorAll('.pose-card')].map((card, index) => ({
                id: parseInt(card.getAttribute('data-id')),
                rank: index + 1
            }));

            fetch('{{ route("fashion-studio.update-pose-rank") }}', {
                    method: 'POST',
                    headers: {
                ...
                .then(data => {
                    showToast(
                        data.success ? '{{ __("Order saved!") }}' : (data.message || '{{ __("Failed to save order.") }}'),
        ...
        // ===== FORM SUBMIT =====
        document.getElementById('create-pose-form').addEventListener('submit', async function(e) {
            e.preventDefault();
            ...
            try {
                ...
                if (response.ok) {
                    showToast(data?.message || '{{ __("Pose created successfully!") }}', 'success');
                    ...
                    btnText.textContent = '{{ __("Create New Pose") }}';
                }
            } catch (err) {
