...
class FalAIWebhookController extends Controller
{
    public function __invoke(Request $request): JsonResponse
    {
        ...
        if (! empty($images) && is_array($images)) {
            ...
            \Log::info('FalAIWebhookController: downloading image', [
                'request_id' => $request->get('request_id'),
                'user_id'    => $openai->user_id,
                'image_url'  => $image,
            ]);

            $image = FluxProQueueCheck::downloadImageToStorage($image, null, $openai->user_id);
        ...
        $singleImage = data_get($payload, 'image.url');
        if ($singleImage) {
            \Log::info('FalAIWebhookController: downloading single image', [
                'request_id' => $request->get('request_id'),
                'user_id'    => $openai->user_id,
                'image_url'  => $singleImage,
            ]);

            $image = FluxProQueueCheck::downloadImageToStorage($singleImage, null, $openai->user_id);
