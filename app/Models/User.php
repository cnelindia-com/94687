...
class User extends Authenticatable
{
    ...
    public function activePlan()
    {
        ...
        if (empty($activeSub)) {
            // If superadmin has no subscription, check for default plan
            if ($this->isSuperAdmin()) {
                $defaultPlan = Plan::getCache(
                    static fn () => Plan::query()->where('default_plan', true)->first(),
                    '-default_plan'
                );
     ...
     * Counts messages where sender is 'user' and is_read is 0.
     */
    public function unreadSupportMessagesCount(): int
    {
        return UserSupportMessage::where('sender', 'user')
            ->where('is_read', 0)
            ->count();
    }

    /**
     * Get total unread admin replies count for a specific user.
     * Counts messages where sender is 'admin' and is_read is 0
     * in the user's support tickets.
     */
    public function unreadAdminRepliesCount(): int
    {
        $ticketIds = $this->supportRequests()->pluck('id');

        if ($ticketIds->isEmpty()) {
            return 0;
        }

        return UserSupportMessage::where('sender', 'admin')
            ->where('is_read', 0)
            ->whereIn('user_support_id', $ticketIds)
            ->count();
    }
    ...
    {
        return $this->myCreatedTeam()->exists();
    }

    public function isTeamMember(): bool
    {
        return $this->teamMember()->exists();
    }

    public function hasTeam(): bool
    {
        return $this->isTeamCreator() || $this->isTeamMember();
    }

    public function effectiveRelationPlan()
    {
        ...
        return $this->relationPlan()->first();
    }

    public function getActivePlanRenewDaysAttribute(): ?int
    {
        if (!$this->isSuperAdmin()) {
            return null;
        }
        ...
        if ($activeSub && $activeSub->ends_at) {
            return max(0, (int) now()->diffInDays($activeSub->ends_at, false));
        }

        // For superadmin with default plan, return a large number indicating no renewal needed
        if ($this->activePlan() && $this->activePlan()->default_plan) {
            return null; // No renewal needed for default plan
        }
