...
use App\Services\Image\WatermarkService;
...
class FluxProQueueCheck extends Command
{
    ...
    public static function updateFluxProImage(?string $request_id = null): void
    {
        ...
        if ($output) {
            ...
            $image = static::downloadImageToStorage($image, null, $item?->user_id);
    ...
    public static function downloadImageToStorage($url = null, $filename = null, ?int $userId = null)
    {
        if (! $url) {
            return null;
        }
        ...
        if ($response->successful()) {
            ...
            if ($userId) {
                ...
                if ($watermark->shouldApply($user)) {
                    $fileContent = $watermark->apply($fileContent);
                }
            }
