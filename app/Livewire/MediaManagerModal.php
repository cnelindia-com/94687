...
class MediaManagerModal extends Component
{
        ...
        return collect($files)
            ...
            ->map(function ($file) use ($disk) {
                ...
                return (object) [
                    ...
                    'format_date' => $createdAt->format('M d, Y'),
                    'source'      => 'uploaded',
                    'creator_name'=> auth()->user()?->fullName() ?? auth()->user()?->name ?? __('Unknown'),
                    'file_path'   => $file,
                ];
            })
            ->values();
    ...
    protected function getImages(bool $getAllForSelection = false): Collection
    {
        $user = auth()->user();
        $userId = $user?->id;
        $teamId = $this->resolveVisibleTeamId($user);
        // Get database images
        ...
        if ($teamId) {
            $query->where(function ($builder) use ($userId, $teamId) {
        ...
        $dbImages = $query->get()->map(function ($item) {
            ...
            $item->creator_name = $item->user?->fullName() ?? $item->user?->name ?? __('Unknown');
        ...
        try {
            ...
            if ($response->successful()) {
                ...
                return (object) [
                    ...
                    'source'      => 'pexels',
                    'creator_name'=> $photo['photographer'] ?? __('Pexels'),
                    'created_at'  => now(),
                    'format_date' => now()->format('M d, Y'),
                ];
            });
                ...
                try {
                    ...
                    $userFallVideos = $userFallQuery->get()->map(function ($item) {
                        ...
                        return (object) [
                            ...
                            'source'      => 'userfall',
                            'creator_name'=> $creator?->fullName() ?? $creator?->name ?? __('Unknown'),
            ...
            try {
                ...
                $exportedVideos = $exportedVideosQuery->get()->map(function ($item) {
                    return (object) [
                        ...
                        'format_date' => $item->created_at->format('M d, Y'),
                        'source'      => 'exported',
                        'creator_name'=> $user?->fullName() ?? $user?->name ?? __('Unknown'),
                        'generator'   => (object) [
                            'id'    => null,
                            'type'  => 'video',
    ...
    private function resolveVisibleTeamId(?User $user): ?int
    {
        if (! $user) {
            return null;
        }
        ...
        return (int) ($user->myCreatedTeam?->id ?: 0) ?: null;
    }
}
