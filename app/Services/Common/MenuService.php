...
class MenuService
{
    ...
    public function generate(bool $active = true): array
    {
        ...
        // Add unread support messages badge for admin and user
        if (auth()->check()) {
            if (auth()->user()->isAdmin()) {
                // Admin: count unread user messages
                $unreadCount = auth()->user()->unreadSupportMessagesCount();
                if ($unreadCount > 0 && isset($data['support_requests'])) {
                    $data['support_requests']['badge'] = (string) $unreadCount;
                }
            } else {
                // User: count unread admin replies
                $unreadCount = auth()->user()->unreadAdminRepliesCount();
                if ($unreadCount > 0 && isset($data['support'])) {
                    $data['support']['badge'] = (string) $unreadCount;
                }
            }
        }
    ...
    public function data(): array
    {
        ...
        $menu = [
            ...
            'ext_fashion_studio_change_model' => [
                ...
                'show_condition' => MarketplaceHelper::isRegistered('fashion-studio') && auth()->check() && (auth()->user()->isSuperAdmin() || (auth()->user()->relationPlan && auth()->user()->relationPlan->change_models)),
                'badge'          => 'new',
            ],
    ...
    && (
        ...
        || (auth()->user()->relationPlan?->create_video)
        || (
            auth()->user()->isTeamMember()
            && auth()->user()->teamMember?->team?->user?->relationPlan?->create_video
        )
    )
    && auth()->user()->teamMember?->team_role !== 'viewer',
                'badge'          => 'new',
            ],
            ...
            'team_menu' => [
                ...
                'show_condition' => $this->showTeamFunctionality() && !auth()->user()?->team_id,
            ],
            'brand_voice' => [
                'parent_key'       => null,
                'key'              => 'brand_voice',
                ...
                'show_condition' =>  auth()->check() && auth()->user()->isSuperAdmin(),
            ],
            'advanced_image' => [
                'parent_key'       => null,
                'key'              => 'advanced_image',
            ...
            // 'support_settings_link' => [
            //     'parent_key'       => null,
            //     'key'              => 'support_settings_link',
            //     'route'            => 'dashboard.support.settings',
            //     'route_slug'       => null,
            ...
            //     'extension'        => null,
            //     'active_condition' => [],
            //     'show_condition'   => auth()->check() && auth()->user()->type === 'super_admin',
            // ],
            'integration' => [
                'parent_key'       => null,
                'key'              => 'integration',
                ...
                'show_condition' => Integration::getCache(
                    static fn () => Integration::query()->whereHas('hasExtension')->count(),
                    'count'
                ),
            ],
            'divider_one' => [
            ...
            //     'route'            => 'dashboard.support.settings',
            //     'label'            => 'Support Settings',
            //     'data-name'        => Introduction::ADMIN_SUPPORT_SETTINGS,
            //     'icon'             => 'tabler-settings',
            //     'svg'              => null,
            //     'order'            => 37,
            //     'is_active'        => true,
    ...
    public static function planAiToolsMenu(): array
    {
        $keys = [
            ...
            'ext_ai_music_pro', 'ai_influencer', 'creative_suite', 'url_to_video', 'viral_clips', 'influencer_avatar',  'support', 'ai_chat_pro_image_chat', 'ai_image_pro',
        ];
