...
class StripeService
{
    ...
    public static function getSubscriptionStatus($incomingUserId = null)
    {
        ...
        if ($sub != null) {
            // Auto-assigned trials are local subscriptions (TRL-*), not Stripe
            // subscriptions. Do not verify them against Stripe, otherwise the
            // failed API lookup marks a valid trial as cancelled.
            if (
                strtolower((string) $sub->stripe_status) === 'trialing'
                && $sub->trial_ends_at !== null
                && Carbon::now()->lt(Carbon::parse($sub->trial_ends_at))
            ) {
                return true;
            }
