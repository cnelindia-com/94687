            ...
            return Subscription::query()
                ...
                ->where(function ($query) {
                    $query->whereIn('stripe_status', [
                        ...
                        $trialQuery->where('stripe_status', 'trialing')
                            ->whereNotNull('trial_ends_at')
                            ->where('trial_ends_at', '>', now());
                    });
                })
                // A paid subscription must take precedence over a trial.
                ->orderByRaw("CASE WHEN stripe_status = 'trialing' THEN 1 ELSE 0 END")
                ->orderByDesc('id')
                ->first();
        });
    }
