...
{
    // First try to get from active subscription
    ...
    // If no subscription, check if current user is superadmin with default plan
...
function ThumbImage($source_file, $max_width = 800, $max_height = 800, $quality = 80, $convert_to_webp = true)
{
    ...
    try {
        ...
        if (in_array($extension, ['heic', 'heif'], true)) {
            ...
            $signature = 'v3|' . $source_file . '|' . filemtime($source_file) . '|' . $max_width . 'x' . $max_height . '|webp:' . (int) $convert_to_webp;
            ...
            try {
                ...
                $imagick->thumbnailImage($max_width, $max_height, true, true);
                ...
                $candidatePath = $dst_dir . $dst_file_name . '.' . $candidateExtension;
                $thumbDisk->put($candidatePath, $imagick->getImageBlob());
                $imagick->clear();
                $imagick->destroy();
        ...
        if (in_array($mime, ['image/heic', 'image/heif', 'image/heif-sequence', 'image/heic-sequence'], true)) {
            $rawBinary = @file_get_contents($source_file);
            $src_img = $rawBinary !== false ? @imagecreatefromstring($rawBinary) : false;
        } else {
...
if (! function_exists('isFileSecure')) {
    ...
    if (! function_exists('validateUploadedFile')) {
        function validateUploadedFile(string $filePath, string $extension): bool
        {
            ...
            // For common raster formats, validate image structure via getimagesize().
            // Note: HEIC/HEIF often isn't supported by getimagesize() without additional extensions,
            // so we validate those via MIME checks instead.
            if (in_array($extension, ['jpg', 'jpeg', 'png', 'gif', 'webp'], true)) {
                $imageInfo = @getimagesize($filePath);
                if ($imageInfo === false) {
                    return false;
                }
            } elseif (in_array($extension, ['heic', 'heif'], true)) {
                $finfo = new finfo(FILEINFO_MIME_TYPE);
                $mime  = $finfo->file($filePath);

                // Some environments report HEIC/HEIF as generic image/* or application/octet-stream.
                // Prefer explicit HEIC/HEIF mimes, but allow image/* as a fallback.
                if (! is_string($mime) || (! str_starts_with($mime, 'image/') && ! in_array($mime, ['application/octet-stream'], true))) {
                    return false;
                }
            }
...
if (! function_exists('processSecureFileUpload')) {
    function processSecureFileUpload($file, string $basePath): ?string
    {
        ...
        if (in_array($extension, ['heic', 'heif'], true)) {
            $convertedUrl = convertHeicUploadToJpeg($file, $basePath);

            if ($convertedUrl) {
                return $convertedUrl;
            }

            // Fall back to storing the original HEIC so upload does not fail.
            // The UI can still surface a placeholder or download link if the server
            // does not have HEIC conversion support installed.
        }
...
if (! function_exists('convertHeicUploadToJpeg')) {

    function convertHeicUploadToJpeg($file, string $basePath): ?string
    {
            if (! class_exists('Imagick') || ! extension_loaded('imagick')) {
            \Log::warning('Imagick not available for HEIC conversion, storing original file instead.');

            $relativePath = trim($basePath, '/') . '/' . time() . '.' . strtolower($file->getClientOriginalExtension());
            ...
            if (is_uploaded_file($file->getRealPath())) {
                move_uploaded_file($file->getRealPath(), $absolutePath);
            } else {
...
/**
 * Get all user IDs belonging to the same team.
 */
function getTeamUserIds(?int $userId = null): array
{
    ...
    // Case 1: User is a team member (has team_manager_id)
    if ($user->team_manager_id && $user->team_id) {
        $teamMemberIds = \App\Models\Team\TeamMember::where('team_id', $user->team_id)
            ->whereNotNull('user_id')
            ->where('status', 'active')
            ->pluck('user_id')
            ->toArray();

        $allIds = array_merge([$user->team_manager_id], $teamMemberIds);
        return array_unique($allIds);
    }

    // Case 2: User is a team leader (check if a Team record exists with user_id = $userId)
    // OR if other users have this user as team_manager_id
    $team = \App\Models\Team\Team::where('user_id', $userId)->first();
    $teamId = null;
    
    if ($team) {
        $teamId = $team->id;
    } else {
        // Fallback: check if this user is referenced as team_manager_id by any other user
        $teamFromMembers = \App\Models\Team\TeamMember::where('email', $user->email)
            ->orWhereIn('user_id', [$userId])
            ->first();
        if ($teamFromMembers) {
            $teamId = $teamFromMembers->team_id;
        }
    }

    if ($teamId) {
        $teamMemberIds = \App\Models\Team\TeamMember::where('team_id', $teamId)
            ->whereNotNull('user_id')
            ->where('status', 'active')
            ->pluck('user_id')
            ->toArray();
...
/**
 * Get creator's full name from user_id.
 */
function getCreatorName(int $userId): string
{
    $user = \App\Models\User::find($userId);
    if ($user) {
...
if (! function_exists('resolveActualStorageUrl')) {
    /**
     * If the given URL is wrapped by our internal proxy-image route,
     * extract and return the real underlying storage URL.
     * Otherwise, return the URL unchanged.
     */
    function resolveActualStorageUrl(?string $url): ?string
    {
        if (empty($url)) {
            return $url;
        }
        // Check if this is our proxy route: .../proxy-image?url=ENCODED_REAL_URL
        if (str_contains($url, 'proxy-image') || str_contains($url, 'proxy/image')) {
            $parsedUrl = parse_url($url);
            if (! empty($parsedUrl['query'])) {
                parse_str($parsedUrl['query'], $queryParams);
                if (! empty($queryParams['url'])) {
                    \Log::info('Unwrapped proxy URL to real storage URL', [
                        'proxy_url' => $url,
                        'real_url'  => $queryParams['url'],
                    ]);
...
if (!function_exists('deleteFileFromOutputUrl')) {

    function deleteFileFromOutputUrl(?string $url): bool
    {
        if (empty($url)) {
            \Log::warning('deleteFileFromOutputUrl called with empty url');
            return false;
        }
        ...
        try {
            ...
            \Log::info('deleteFileFromOutputUrl started', [
                'url'      => $url,
                'path'     => $path,
                'filename' => $filename,
            ]);

            // ✅ Check if the URL is from DigitalOcean Spaces (S3)
           // ✅ Check if the URL is from DigitalOcean Spaces (S3)
$s3UrlPattern = '/digitaloceanspaces\.com/i';
$awsUrlPattern = '/s3\.amazonaws\.com/i';
if (preg_match($s3UrlPattern, $url) || preg_match($awsUrlPattern, $url) || str_contains($url, env('AWS_URL', ''))) {
    try {
        $s3Path = ltrim(parse_url($url, PHP_URL_PATH) ?: '', '/');
        $s3Path = rawurldecode($s3Path);
        $s3Path = preg_replace('#/+#', '/', $s3Path);

        // ✅ FIX: If using path-style endpoint, the bucket name is the FIRST
        // segment of the URL path — strip it, because Storage::disk('s3')
        // already knows the bucket from config and expects a bucket-relative key.
        $bucket = config('filesystems.disks.s3.bucket');
        if ($bucket && (str_starts_with($s3Path, $bucket . '/') || $s3Path === $bucket)) {
            $s3Path = ltrim(substr($s3Path, strlen($bucket)), '/');
        }

        \Log::info('Attempting to delete from S3', [
            's3_path' => $s3Path,
            'url'     => $url,
            'bucket'  => $bucket,
        ]);
        ...
        if (\Illuminate\Support\Facades\Storage::disk('s3')->exists($s3Path)) {
            ...
            \Log::info('File deleted successfully from DigitalOcean Spaces (S3)', [
                'key' => $s3Path,
                'url' => $url,
            ]);
        } else {
            \Log::warning('File not found on DigitalOcean Spaces (S3)', [
                'key' => $s3Path,
                'url' => $url,
            ]);
        }
    } catch (\Exception $e) {
        \Log::error('Failed to delete file from DigitalOcean Spaces (S3)', [
            'key'   => $s3Path ?? $path,
            'url'   => $url,
            'error' => $e->getMessage(),
        ]);
    ...
    // Still attempt local deletion as well
}
            ...
            $relativeCandidates = [
                $path,
                $basename,
                str_contains($path, '/uploads/') ? substr($path, strpos($path, '/uploads/') + 1) : null,
                str_contains($path, 'uploads/') ? substr($path, strpos($path, 'uploads/')) : null,
                str_contains($path, '/public/uploads/') ? substr($path, strpos($path, '/public/uploads/') + 1) : null,
                str_contains($path, 'public/uploads/') ? substr($path, strpos($path, 'public/uploads/')) : null,
            ];

            $candidates = array_values(array_unique(array_filter(array_map(function ($candidate) {
            ...
            foreach ($candidates as $candidate) {
                ...
                $fullPath = is_file($candidate) ? $candidate : public_path($candidate);
                \Log::info('Checking file for deletion: ' . $fullPath);

                if (is_file($fullPath)) {
                    unlink($fullPath);
                    \Log::info('File deleted successfully from filesystem');
                    $deleted = true;
                }

                $publicDiskPath = preg_replace('#^public/#', '', $candidate);
                $publicDiskPath = ltrim($publicDiskPath, '/');
                if (Storage::disk('public')->exists($candidate)) {
                    Storage::disk('public')->delete($candidate);
                    \Log::info('File deleted successfully from public disk: ' . $candidate);
                    $deleted = true;
                }

                if ($publicDiskPath && $publicDiskPath !== $candidate && Storage::disk('public')->exists($publicDiskPath)) {
                    Storage::disk('public')->delete($publicDiskPath);
                    \Log::info('File deleted successfully from public disk: ' . $publicDiskPath);
                    $deleted = true;
                }
            }

            $thumbnailPatterns = [
                public_path('uploads/thumbnail/default/' . $filename . '*'),
                public_path('uploads/thumbnail/' . $filename . '*'),
                public_path('uploads/thumbnail/**/' . $filename . '*'),
            ];

            $originalPatterns = [
                public_path('uploads/media/**/' . $filename . '*'),
                public_path('uploads/media/images/**/' . $filename . '*'),
                public_path('uploads/media/images/u-*/' . $filename . '*'),
            ];

            foreach ($thumbnailPatterns as $pattern) {
                \Log::info('Checking thumbnail pattern for deletion: ' . $pattern);
            ...
            foreach ($originalPatterns as $pattern) {
                \Log::info('Checking original pattern for deletion: ' . $pattern);

                foreach (glob($pattern, GLOB_BRACE) ?: [] as $originalPath) {
                    if (is_file($originalPath)) {
                        \Log::info('Deleting original image: ' . $originalPath);
                        unlink($originalPath);
                        $deleted = true;
                    }
                }
            ...
            if (! $deleted) {
                \Log::warning('File not found for deletion: ' . $url);
            }
